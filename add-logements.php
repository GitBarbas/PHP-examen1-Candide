...
if (!empty($_POST)) {
    ...
    $response->execute([
        ...
        $newName = 'product_' . $id_logement;
        if ($_FILES['photo']['error'] == 0) {
        // Testons si le fichier n'est pas trop gros
        if ($_FILES['photo']['size'] <= 32000000) {
            // Testons si l'extension est autorisée
            $infosfichier = pathinfo($_FILES['photo']['name']);
            $extension_uploads = $infosfichier['extension'];
            $extensions_autorisees = ['jpg', 'jpeg', 'gif', 'png'];
            if (in_array($extension_uploads, $extensions_autorisees)) {
                // On peut valider le fichier et le stocker définitivement
                move_uploaded_file($_FILES['photo']['tmp_name'], 'uploads/' .  $newName . '.' . $extension_uploads);
                echo "L'envoi a bien été effectué !";

                // pour que l'image s'affiche dans le fichier list-logement
                $request = 'UPDATE logement
                            SET photo = "' . $newName . '.' . $extension_uploads . '" 
                 ...
                 */
                $titreAncienneImage = $newName . '.' . $extension_uploads;       // Le nom de l'image de départ AVEC extension
                $extension = $extension_uploads;                                 // L'extension de départ
